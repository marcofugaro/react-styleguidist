...
function examplesLoader(source) {
  ...
  const _ref = loaderUtils.getOptions(this) || {},
        file = _ref.file,
        displayName = _ref.displayName,
        shouldShowDefaultExample = _ref.shouldShowDefaultExample; // Replace placeholders (__COMPONENT__) with the passed-in component name
  ...
  const examples = chunkify(source, updateExample); // Find all import statements and require() calls in examples to make them
  ...
  const allCodeExamples = filter(examples, {
    type: 'code'
  });
  ...
  const fullContext = _objectSpread({}, config.context, {
  ...
  const allModules = [...requiresFromExamples, ...values(fullContext)]; // “Prerequire” modules required in Markdown examples and context so they
  ...
  const requireContextCode = b.program( // const name = require(path)
  map(fullContext, (requireRequest, name) => b.variableDeclaration('const', [b.variableDeclarator(b.identifier(name), requireIt(requireRequest).toAST())]))); // Stringify examples object except the evalInContext function
